...
export async function read({cel, trustedWitnesses = [], versionTime = null}) {
  ...
  for(let i = 0; i < cel.log.length; i++) {
    ...
    // 1. Filter witness proofs to only those from trusted witnesses whose
    ...
    // 2. Verify previousEventHash for all entries after the first
    if(i > 0) {
    ...
    // Track the current DID document state; heartbeat events carry only a
    // partial update (just the new heartbeat array), so merge rather than replace
    if(event.operation?.data) {
      if(event.operation.type === 'heartbeat') {
        currentDidDocument = {
          ...currentDidDocument,
          heartbeat: event.operation.data.heartbeat
        };
      } else {
    ...
    // 3. Verify the operation proof.
    // Every event must carry a proof - a missing proof is always a hard error,
    // not a no-op. Relying solely on the JSON Schema required check in
    // assertValidCel() is insufficient: that check runs once on the whole CEL
    // structure, but the security invariant must also be enforced here so that
    // no code path can accept an unsigned event as valid.
    ...
    if(!opProof) {
      errors.push(`entry ${i}: operation proof is missing`);
    } else {
      try {
    ...
    // 4. Verify each trusted witness proof and check timestamp deviation
    ...
    // 5. For every operation except create (i===0) and deactivate, verify that
    // the signing heartbeat key's hash has been rotated out and a new one added.
    // Deactivate is terminal so no rotation is needed; the create event
    ...
    if(opProof && currentDidDocument) {
      ...
      if(vmRef?.startsWith('did:key:') &&
          i > 0 && event.operation?.type !== 'deactivate') {
    ...
    // 6. Check heartbeatFrequency: for each entry after the first, the elapsed
    ...
    // If heartbeatFrequency is not set, the default is P1M (1 month).
