 ...
 *
 * @param {object} options - Configuration options.
 * @param {object} options.cel - The parsed Cryptographic Event Log object.
 * @param {Array<object>} [options.trustedWitnesses=[]] - Trusted witness entries.
 *   Each entry: {id: <witness DID>, validFrom: <ISO datetime>,
 *   validUntil: <ISO datetime>}. Only witness proofs whose verificationMethod
 *   DID matches an entry and whose created falls within validFrom/validUntil
 *   are verified. Proofs from unknown witnesses are silently ignored.
...
export async function read({cel, trustedWitnesses = [], versionTime = null}) {
  ...
  const firstDidDocument = JSON.parse(
    JSON.stringify(firstEvent?.operation?.data ?? {}));
  ...
  const mfHash = await sha3256Hasher.digest(
    utf8Encoder.encode(canonicalize(firstDidDocument))).bytes;
  const expectedId = 'did:cel:' + base58btc.encode(mfHash);
  ...
  for(let i = 0; i < cel.log.length; i++) {
    ...
    const trustedWitnessProofs = witnessProofs.filter(wp => {
      ...
      const validFrom = entry.validFrom ? new Date(entry.validFrom).getTime() : null;
    ...
    for(let j = 0; j < trustedWitnessProofs.length; j++) {
      ...
      if(!witnessProof.created) {
        errors.push(`entry ${i} witness ${j}: missing required created timestamp`);
      } else {
        const wTime = new Date(witnessProof.created).getTime();
        // always track the latest trusted witness timestamp for heartbeat checks
        if(entryWitnessTime === null || wTime > entryWitnessTime) {
          entryWitnessTime = wTime;
        }
        // 4. Check timestamp deviation ≤ 5 minutes (requires operation proof time)
        if(opTime !== null) {
...
export default {addEvent, create, loadFromFile, read, witness};
