...
import {VERIFICATION_RELATIONSHIPS, sha3256Multibase} from './utils.js';
...
export async function witness({cel, witnesses}) {
  ...
  // hash the bare event (not the wrapper) — this is what the spec requires
 ...
 * @returns {Promise<string|undefined>} Multibase hash, or undefined if empty.
 */
 ...
 *   6. Witness proofs — signature + ≤5 min clock deviation per trusted witness.
 ...
 * @param {Array<object>} [options.trustedWitnesses=[]] - Trusted witness
 *   entries: `{id, validFrom, validUntil}`. Proofs from unknown witnesses or
 *   outside the validity window are ignored.
 * @param {string|null} [options.versionTime=null] - ISO datetime. When set,
 *   entries whose earliest trusted witness timestamp exceeds this value are
...
export async function read({cel, trustedWitnesses = [], versionTime = null}) {
  ...
  for(let i = 0; i < cel.log.length; i++) {
    ...
    // needs the frequency that was in effect for the gap leading into this entry,
    // not any new value this entry's update might introduce.
    ...
    // Look up keys in the *prior* document to prevent circular key-introduction:
    // an attacker must not be able to add a key via an update and simultaneously
    // use that key to sign the update. The create event (i === 0) has no prior
    // state; its document integrity is already pinned by the self-certifying ID.
...
async function _verifyHashChain({cel, i, event}) {
  const computed = await sha3256Multibase(canonicalize(cel.log[i - 1].event));
 ...
 * @param {object} options.verifyDidDocument - Document to look up signing keys in.
...
/**
 * Verifies all trusted witness proofs for a single log entry.
...
async function _checkHeartbeatRotation(
  ...
  const usedHash = await sha3256Multibase(didKeyId);
 ...
 * @param {object} options
 * @param {number} options.i - Log entry index.
 * @param {number|null} options.prevEntryWitnessTime - Latest witness ms from prior entry.
...
/**
 * Returns true if `wp` was issued by a trusted witness and its `created`
...
async function _verifyOperationProof({event, opProof, prevDidDocument}) {
  ...
  const didKeyId = vmRef.split('#')[0];
  const hash = await sha3256Multibase(didKeyId);
