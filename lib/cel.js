...
import {
  sha2256Multibase, sha3256Multibase, VERIFICATION_RELATIONSHIPS
} from './utils.js';
 ...
 * The event is hashed (SHA2-256 multihash — the digest substitutes for the
 * SHA-256 document hash in the witness's data integrity proof) and the
 * digest is sent to each witness URL with a requested cryptosuite of
 * `ecdsa-jcs-2019`; each witness returns a DataIntegrityProof.
 *
 * @param {object} options - Configuration options.
...
export async function witness({cel, witnesses, allowSelfSigned}) {
  ...
  const digestMultibase = sha2256Multibase(canonicalize(logEntry.event));
 ...
 * VerifyData = SHA256(JCS(proofOptions)) || SHA256(JCS(event)).
 * The raw SHA2-256 digest (not a multihash) is used because the witness
 * receives and signs the digest directly without multihash framing; it
 * substitutes for the cryptosuite's SHA-256 document hash.
 *
 * @param {object} options - Configuration options.
 * @param {object} options.logEntry - The full log entry `{event, proof[]}`.
...
async function _verifyWitnessProof({logEntry, witnessProof}) {
  ...
  const rawHash = new Uint8Array(crypto.createHash('sha256')
    .update(canonicalize(logEntry.event)).digest());
