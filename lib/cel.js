...
async function _verifyHashChain({cel, i, event}) {
  const computed = await sha3256Multibase(
    canonicalize(cel.log[i - 1].event));
 ...
 * This is the first half of verifyData for both proof schemes.
 *
 * @param {object} proof - The proof object.
 * @returns {Uint8Array} SHA-256 hash of the canonicalized proof options.
 */
 ...
 *
 * @param {string} vmId - The full verificationMethod URI (did:key:z…#z…).
 * @returns {Promise<object>} An EcdsaMultikey verifier instance.
 */
 ...
 * where rawHash is the 32-byte SHA3-256 digest of the canonicalized event.
 *
 * @param {object} options - Options.
 * @param {object} options.logEntry - The full log entry {event, proof[]}.
...
async function _verifyWitnessProof({logEntry, witnessProof}) {
  ...
  // reconstruct rawHash from the bare event (same as what was sent to the
  // witness service)
