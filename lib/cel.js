...
export async function load({filename}) {
  ...
  for(let i = 0; i < cel.log.length; i++) {
    ...
    if(i > 0) {
      const computed = await _calculatePreviousEventHash(
 ...
 */
async function _verifyOperationProof({event, opProof, currentDidDocument}) {
  // find the assertionMethod key matching the verificationMethod in the proof
  const vmRef = opProof.verificationMethod;
  const assertionKey = _findAssertionKey(
    {vmRef, didDocument: currentDidDocument});
  if(!assertionKey) {
    throw new Error(`verification method not found in DID document: ${vmRef}`);
  }

  // previousEventHash is appended after signing; exclude from doc hash
  const doc = {...event};
  delete doc.proof;
  delete doc.previousEventHash;
  ...
  const keyPair = await EcdsaMultikey.from({
    type: 'Multikey',
    id: vmRef,
    controller: currentDidDocument.id,
...
async function _verifyWitnessProof({logEntry, witnessProof}) {
  ...
  // reconstruct the digestMultibase from the log entry's event
  // (same as what was sent to the witness service — sans witness proofs)
  const entryForDigest = {event: logEntry.event};
  const canonicalized = canonicalize(entryForDigest);
  const rawHashFull = sha256(utf8Encoder.encode(canonicalized));
