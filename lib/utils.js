...
// module-level hashers — stateless, reusable across all calls
const _sha3256Hasher = mfHasher.from({
  name: 'sha3-256',
  code: 0x16,
  encode: data => sha3_256(data)
});
const _sha2256Hasher = mfHasher.from({
  name: 'sha2-256',
  code: 0x12,
  encode: data => sha256(data)
});
 ...
 * @returns {string} `z`-prefixed base58btc multibase string.
 */
export function sha3256Multibase(input) {
  const {bytes} =
    _sha3256Hasher.digest(new TextEncoder().encode(input));
  return base58btc.encode(bytes);
}

/**
 * Returns the base58btc multibase-encoded SHA2-256 multihash of a string.
 * This is used for witness digests, which substitute for the SHA-256
 * document hash in the witness's data integrity proof.
 *
 * @param {string} input - UTF-8 string to hash.
 * @returns {string} `z`-prefixed base58btc multibase string.
 */
export function sha2256Multibase(input) {
  const {bytes} =
    _sha2256Hasher.digest(new TextEncoder().encode(input));
  return base58btc.encode(bytes);
}
