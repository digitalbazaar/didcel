/**
 * @fileoverview DID CEL (Cryptographic Event Log) DID Document management.
...
export async function create({options}) {
  ...
  let didDocument = {
    ...
    heartbeatFrequency: options?.heartbeatFrequency || 'P3M',
  ...
  recoveryPublicKey.controller = controller;
 ...
 * });
 */
export async function createEvent({data, assertionMethod}) {
  ...
  signedEvent.proof.verificationMethod = assertionMethod.controller + '#' +
    assertionMethod.publicKeyMultibase;

  return {event: signedEvent};
}

export default {create, addVm, createEvent};
