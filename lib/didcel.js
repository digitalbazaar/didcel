...
/**
 * Creates a new did:cel DID document. The DID identifier is derived from
 * SHA3-256(JCS(initial document)) so the identifier is self-certifying.
 * The document contains no assertionMethod keys at creation time; add
 * verification methods afterwards via `addVm()`.
 *
 * @param {object} [options] - Configuration options.
 * @param {string} [options.heartbeatFrequency='P1M'] - ISO 8601 duration.
 * @param {object[]} [options.service] - Service entries to include in the DID
 *   document. Each must be a valid DID Core service object. If omitted, no
 *   `service` property is set on the document.
 * @returns {Promise<object>} `{heartbeatSecret, didDocument,
 *   cryptographicEventLog}`.
 */
export async function create({heartbeatFrequency = 'P1M', service} = {}) {
  // derive the initial heartbeat key from a fresh 256-bit master secret
  const heartbeatSecret = crypto.randomBytes(32);
  ...
  const heartbeatHash = sha3256Multibase(heartbeatDidKey);
  ...
  const encodedHash = sha3256Multibase(canonicalize(didDocument));
  ...
  const signedEvent =
    await _signEvent({event, signer: heartbeatKeyPair.signer()});
 ...
 *
 * @param {Buffer|Uint8Array} masterSecret - 32-byte master secret.
...
export async function deriveHeartbeatKeyPair(masterSecret, index) {
  ...
  const keyPair =
    await EcdsaMultikey.fromRaw({curve: 'P-256', secretKey, publicKey});
...
/**
 * Adds a verification method to the specified relationship on a DID document.
 * The proof is stripped and must be regenerated via `createEvent` before the
 * document is used.
 *
 * @param {object} options - Configuration options.
 ...
 * @param {object} [options.keyPair] - An existing EcdsaMultikey key pair to
 *   add. If omitted, a new one is generated using `curve`.
 * @param {string} [options.curve='P-256'] - Elliptic curve for key generation.
 *   Ignored if `keyPair` is supplied.
 * @returns {Promise<object>} `{keyPair, didDocument}` (no proof attached).
 */
export async function addVm(
  {didDocument, verificationRelationship, keyPair: suppliedKeyPair, curve}) {
  const newDidDocument = structuredClone(didDocument);
  const keyPair = suppliedKeyPair ??
