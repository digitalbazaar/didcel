...
const MULTIBASE_BASE58BTC_STRING =
  {type: 'string', pattern: '^z[1-9A-HJ-NP-Za-km-z]+'};
const DID_CEL = {type: 'string', pattern: '^did:cel:z[1-9A-HJ-NP-Za-km-z]+'};
const XML_DATETIMESTAMP = {
  type: 'string',
  pattern:
...
const VERIFICATION_METHOD = {
  ...
  oneOf: [
    {required: ['publicKeyMultibase'],
      properties: {publicKeyMultibase: MULTIBASE_BASE58BTC_STRING},
      not: {required: ['publicKeyJwk']}},
    {required: ['publicKeyJwk'],
      properties: {publicKeyJwk: {type: 'object'}},
      not: {required: ['publicKeyMultibase']}}
  ],
...
const DID_DOCUMENT_SCHEMA = {
  type: 'object',
  required: ['@context', 'id', 'heartbeatFrequency', 'heartbeat'],
  properties: {
    '@context': {
      type: 'array',
      prefixItems: [
    ...
    authentication: {type: 'array', items: VERIFICATION_METHOD, minItems: 1},
    keyAgreement: {type: 'array', items: VERIFICATION_METHOD, minItems: 1},
    capabilityDelegation: {
      type: 'array', items: VERIFICATION_METHOD, minItems: 1
    },
    ...
    heartbeat: {
      type: 'array',
      items: MULTIBASE_BASE58BTC_STRING,
      minItems: 1
    },
    service: {
      type: 'array',
...
const DATA_INTEGRITY_PROOF = {
  ...
  properties: {
    ...
    created: XML_DATETIMESTAMP,
    proofPurpose: {type: 'string'},
    proofValue: MULTIBASE_BASE58BTC_STRING,
...
const HEARTBEAT_OPERATION = {
  ...
  properties: {
    ...
    data: {
      ...
      properties: {
        heartbeat: {
          type: 'array', items: MULTIBASE_BASE58BTC_STRING, minItems: 1}
      },
      additionalProperties: false
    }
  },
...
const NON_CREATE_EVENT = {
  ...
  properties: {
    '@context': {},
    previousEventHash: MULTIBASE_BASE58BTC_STRING,
    operation: {
      oneOf: [UPDATE_OPERATION, HEARTBEAT_OPERATION, DEACTIVATE_OPERATION]},
    proof: DATA_INTEGRITY_PROOF
  },
  additionalProperties: false
};
